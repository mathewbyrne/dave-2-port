...
static void win32_resize_backbuffer(win32_backbuffer_t *buffer, int width,
                                    int height) {
    if (buffer->memory) {
    ...
    buffer->memory            = VirtualAlloc(0, bitmap_memory_size,
                                             MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
}

static void win32_present_backbuffer(win32_backbuffer_t *buffer,
...
static LRESULT CALLBACK win32_main_window_proc(HWND window, UINT message,
...
int CALLBACK WinMain(HINSTANCE instance, HINSTANCE prev_instance,
    ...
    HWND window = CreateWindowExA(
        0, window_class.lpszClassName, "Dangerous Dave 2 (Port)",
    ...
    while (g_running) {
        ...
        game_tick(&game_state, dt, (uint32_t *)g_backbuffer.memory,
        ...
        win32_present_backbuffer(&g_backbuffer, device_context, window_width,
                                 window_height);
