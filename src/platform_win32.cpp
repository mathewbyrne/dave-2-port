...
static void win32_present_backbuffer(win32_backbuffer_t *buffer, HDC device_context, int window_width, int window_height) {
...
static LRESULT CALLBACK win32_main_window_proc(HWND window, UINT message, WPARAM w_param, LPARAM l_param) {
    switch (message) {
        case WM_CLOSE:
        case WM_DESTROY:
            g_running = 0;
...
int CALLBACK WinMain(HINSTANCE instance, HINSTANCE prev_instance, LPSTR cmd_line, int show_code) {
    ...
    while (g_running) {
        ...
        float dt = (float) (current_counter.QuadPart - last_counter.QuadPart) / (float) frequency.QuadPart;
        ...
        game_tick(&game_state, dt, (uint32_t *)g_backbuffer.memory, g_backbuffer.width, g_backbuffer.height);
